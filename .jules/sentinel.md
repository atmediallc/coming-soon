...
## 2024-07-08 - Early Request Limits Prevent DoS
**Vulnerability:** Next.js API route accepted large payloads into memory.
**Learning:** `request.json()` in Next.js will read the full body into memory. Excessively large payloads can cause memory exhaustion before length validation.
**Prevention:** Enforce `Content-Length` checks directly on `Request.headers` before invoking body parsers.

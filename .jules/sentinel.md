...
## 2026-06-28 - DoS via Huge JSON Payload Memory Exhaustion
**Vulnerability:** Waitlist API endpoint accepted extremely large requests which were parsed fully in memory by `request.json()` and only client-side checks existed on input length, leading to potential memory exhaustion DoS attacks.
**Learning:** Next.js route handlers default to loading the entire request body into memory when calling `request.json()`. Attackers can bypass client-side validation and send massive JSON payloads, crashing the Node.js process before application-level validation runs.
**Prevention:** Implement early `Content-Length` checks on Next.js API routes before accessing `request.json()` and couple it with `maxLength` attributes on client-side form inputs to defend against DoS at both layers.

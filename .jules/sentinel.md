...
## 2025-02-27 - Multiple small security improvements
**Vulnerability:** Information disclosure via headers, potential resource exhaustion via large payloads, and missing client-side limits.
**Learning:** Security requires defense in depth across different layers (config, API, client).
**Prevention:**
1. Use `poweredByHeader: false` in `next.config.ts`.
2. Add early `Content-Length` bounds checks on API inputs.
3. Consistently use `maxLength` on user input fields to avoid unintended algorithmic complexity.

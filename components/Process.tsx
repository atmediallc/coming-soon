...
// ⚡ Bolt Optimization: Pre-calculate static outcomes list outside component render.
// This avoids recreating the array of strings and allocating memory on every render.
const KEY_OUTCOMES = [
...
// ⚡ Bolt Optimization: Pre-calculate complex Tailwind class strings and static step structure outside
...
const PRE_CALCULATED_STEPS = PROCESS_STEPS_REGULAR.map((step, i, arr) => ({
  ...step,
  hasConnector: i < arr.length - 1,
  numberClass: cn(
          ...
          <div className="flex-1 w-full max-w-lg">
            <div className="relative glass p-8 md:p-10 rounded-3xl border border-white/5 overflow-hidden">
              ...
              <ol className="relative flex flex-col space-y-5" aria-label="TraderAdd workflow steps">
                {PRE_CALCULATED_STEPS.map((step, i) => (

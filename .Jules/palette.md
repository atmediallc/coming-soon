...
## 2024-07-04 - Dark Mode WCAG AA Color Contrast
**Learning:** Using low opacity utilities like `text-white/30` or `text-white/40` for secondary text or placeholders on dark backgrounds fails WCAG AA contrast ratios, negatively impacting readability and accessibility.
**Action:** Always ensure that secondary text and placeholders use at least `text-white/50` (or an equivalent `text-muted` utility) to maintain sufficient contrast on dark backgrounds.
